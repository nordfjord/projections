...
export type EventFunc<TContext, TReturn = void, TEvent = any> = (event: any, context: TContext) => TReturn | Promise<TReturn>
export type EventHandler<TContext, TEvent = any> = EventFunc<TContext, void, TEvent>
export type EventPredicate<TContext, TEvent = any> = EventFunc<TContext, boolean, TEvent>
...
export class EventMap<TContext> implements IEventMap<TContext> {
  ...
  add<TEvent>(Event: Type<TEvent> | Type<TEvent>[], action: EventFunc<TContext, void, TEvent>) {
  ...
  private async passesFilter(event: any, context: TContext) {
    ...
    const results = await Promise.all (this.filters.map (filter => filter (event, context)))
