...
import { EntityProjectorMap, IEntityProjectorMap, ProjectorMap } from './projector-map'
...
export class EventMapBuilder<TContext> implements IEventMapBuilder<TContext> {
  ...
  public map<TEvent>(...Events: Type<TEvent>[]) {
    return new Action (Events, () => this.eventMap, () => this.projector)
  }
...
export interface IEntityEventMapBuilder<TProjection, TKey, TContext> {
  build(projector: EntityProjectorMap<TProjection, TKey, TContext>): IEventMap<TContext>
}
...
export class EntityEventMapBuilder<TProjection, TKey, TContext>
  ...
  public where(predicate: EventPredicate<TContext>): EntityEventMapBuilder<TProjection, TKey, TContext> {
  ...
  public map<TEvent>(Event: Type<TEvent>) {
    return new CrudAction (Event, this.eventMapBuilder.map (Event), () => this.projector)
  }

  build(projector: IEntityProjectorMap<TProjection, TKey, TContext>): IEventMap<TContext> {
    this.projector = projector
...
class Action<TEvent, TContext> implements IAction<TEvent, TContext> {
  ...
  constructor(private readonly Event: Type<TEvent> | Type<TEvent>[],
  ...
  as(action: EventHandler<TContext, TEvent>): void {
    this.add ((event, context) => this.getProjector ().custom (context, async () => await action (event, context)))
  }
...
class CrudAction<TEvent, TProjection, TKey, TContext> {
  constructor(private readonly Event: Type<TEvent>,
              private readonly actionBuilder: Action<TEvent, TContext>,
              private readonly getProjector: () => EntityProjectorMap<TProjection, TKey, TContext>) {
  }

  public asCreateOf(getKey: GetKey<TEvent, TContext, TKey>) {
    return new CreateAction (this.Event, getKey, this.actionBuilder, this.getProjector)
  }

  public asUpdateOf(getKey: GetKey<TEvent, TContext, TKey>) {
    return new UpdateAction (this.Event, getKey, this.actionBuilder, this.getProjector)
  }

  public asDeleteOf(getKey: GetKey<TEvent, TContext, TKey>) {
    return new DeleteAction (this.Event, getKey, this.actionBuilder, this.getProjector)
  }

  public as(action: (event: TEvent, context: TContext) => Promise<void> | void) {
    this.actionBuilder.as ((event, context) => this.getProjector ().custom (context, () => action (event, context)))
  }
}

class CreateAction<TEvent, TProjection, TKey, TContext> {
  private shouldOverwrite: (projection: TProjection, event: TEvent, context: TContext) => boolean =
    (projection, event, context) => {
      throw new Error (`Projection with key ${this.getKey (event, context)} already exists`)
    }
  ...
  public using(projector: (projection: TProjection, event: TEvent, context: TContext) => Promise<void> | void) {

    this.actionBuilder.as ((event, context) => this.getProjector ().create (
      this.getKey (event, context),
      context,
      projection => projector (projection, event, context),
      existingProjection => this.shouldOverwrite (existingProjection, event, context)
    ))
  ...
  public handlingDuplicatesUsing(shouldOverwrite: (projection: TProjection, event: TEvent, context: TContext) => boolean) {
...
class UpdateAction<TEvent, TProjection, TKey, TContext> {
  ...
  public handlingMissesUsing(handler: (key: TKey, context: TContext) => boolean) {
...
class DeleteAction<TEvent, TKey, TContext> {
  private handleMissing: (key: TKey, context: TContext) => void | Promise<void> =
    (key: TKey, context: TContext) => {
      throw new Error (`Failed to delete projection with key ${key} because it does not exist`)
    }

  constructor(private readonly Event: Type<TEvent>,
              private readonly getKey: GetKey<TEvent, TContext, TKey>,
              private readonly actionBuilder: Action<TEvent, TContext>,
              private readonly getProjector: () => EntityProjectorMap<any, TKey, TContext>) {
  ...
  public handlingMissesUsing(handler: (key: TKey, context: TContext) => void | Promise<void>) {
