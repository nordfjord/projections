...
export class TypeOrmChildProjector<TProjection, TKey> implements ITypeOrmChildProjector {
  ...
  constructor(private readonly Projection: Type<TProjection>,
    ...
    this.mapConfigurator = new TypeOrmEventMapConfigurator<TProjection, TKey> (Projection, mapBuilder, setKey, children)
  }
  ...
  async projectEvent(event: any, context: TypeOrmContext): Promise<void> {
    try {
      ...
      const exception = new ProjectionException ('Projector failed to project an event', err)
