...
export class TypeOrmEventMapConfigurator<TProjection, TKey> {
  ...
  private buildMap(builder: IEntityEventMapBuilder<TProjection, TKey, TypeOrmContext>) {
  ...
  async create(key: TKey, context: TypeOrmContext, projector: Action<TProjection>, shouldOverwrite: Predicate<TProjection>) {
  ...
  async update(key: TKey, context: TypeOrmContext, projector: Action<TProjection>, createIfMissing: () => boolean): Promise<void> {
    const repo = await context.getRepository (this.Projection)
    let projection = await repo.findOne (key)
