...
export class TypeOrmProjector<TProjection, TKey> {
  private readonly mapConfigurator: TypeOrmEventMapConfigurator<TProjection, TKey>
  private readonly children: ITypeOrmChildProjector[]
  public shouldRetry: ShouldRetry = () => Promise.resolve (false)

  constructor(private readonly Projection: Type<TProjection>,
    ...
    this.mapConfigurator = new TypeOrmEventMapConfigurator (Projection, mapBuilder, setKey, children)
  ...
  private async projectEventBatch(events: EventEnvelope[]) {
    for (const envelope of events) {
      try {
        await this.projectEvent (envelope.body, { getRepository: this.repositoryFactory })
      } catch (err) {
        const exception = err instanceof ProjectionException ? err : new ProjectionException ('Projector failed to project an event.', err)
