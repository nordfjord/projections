...
test ('dispatcher restarts when ahead', async () => {
  ...
  const createSubscription = jest.fn (async (lastCheckpoint: bigint, handler: (eb: EventEnvelope) => Promise<void>, id: string) => {
  ...
  expect (createSubscription).toHaveBeenCalledWith (1n, expect.anything (), expect.anything ())
  expect (createSubscription).toHaveBeenCalledWith (-1n, expect.anything (), expect.anything ())
})

test ('dispatcher follows the exception policy', async (done) => {
  const subject = new Subject<EventEnvelope> ()
  const createSubscription = async (lastCheckpoint: bigint, handler: (event: EventEnvelope) => Promise<void>, id: string) => subject
    .pipe (concatMap (handler))
    .subscribe ()
  const dispatcher = new Dispatcher (createSubscription)
  const handler = jest.fn ().mockRejectedValueOnce (new Error ('failure'))
  const subscription = await dispatcher.subscribe (-1n, handler) as Subscription
