import { EntityEventMapBuilder, EntityProjectorMap, EventMapBuilder } from '../src'
...
test ('Creation', async () => {
  ...
  builder.map (ProductAddedToCatalog).asCreateOf (event => event.productKey)
    .using ((p, e) => {
      p.category = e.category
    })

  const map = builder.build (new class extends EntityProjectorMap<ProductCatalogEntry, string, any> {
    async create(key: string, ctx: any,
                 projector: Action<ProductCatalogEntry>,
                 shouldOverwrite: (p: ProductCatalogEntry) => boolean): Promise<void> {
  ...
  // When
  await map.handle (plainToClass (ProductAddedToCatalog, {
    category: 'Hybrid',
    productKey: 'key1'
  }), {})
...
test ('Creation from context', async () => {
  ...
  const builder = new EntityEventMapBuilder<ProductCatalogEntry, string, Context> ()
  builder.map (ProductAddedToCatalog).asCreateOf ((_, ctx) => ctx.metadata.productId)
    .using ((p, e) => {
      p.category = e.category
    })

  const map = builder.build (new class extends EntityProjectorMap<ProductCatalogEntry, string, Context> {
    async create(key: string, context: any,
                 projector: Action<ProductCatalogEntry>,
                 shouldOverwrite: Predicate<ProductCatalogEntry>): Promise<void> {
      projection = new ProductCatalogEntry ()
      projection.id = key
      await projector (projection)
    }
...
test ('Creation when exists is ignored', async () => {
  ...
  const map = builder.build (new class extends EntityProjectorMap<ProductCatalogEntry, string, any> {
    async create(key: string, context: any, projector: Action<ProductCatalogEntry>, shouldOverwrite: Predicate<ProductCatalogEntry>): Promise<void> {
...
const projector = new class extends EntityProjectorMap<ProductCatalogEntry, string, any> {
  async create(key: string, context: any, projector: Action<ProductCatalogEntry>, shouldOverwrite: Predicate<ProductCatalogEntry>): Promise<void> {
    let projection: ProductCatalogEntry
  ...
  async update(key: string, context: any, projector: Action<ProductCatalogEntry>, createIfMissing: () => boolean): Promise<void> {
  ...
  async custom(context: any, projector: () => (Promise<void> | void)): Promise<void> {
    await projector ()
  }
}
...
describe ('Complete handling with defaults', () => {
  ...
  test ('It creates an entry', async () => {
    await map.handle (plainToClass (ProductAddedToCatalog, { category: 'Category', productKey: '1' }), {})

    expect (db['1']).toMatchObject ({ id: '1', category: 'Category' })
  })

  test ('it throws creating the same entry', async () => {
    await expect (map.handle (plainToClass (ProductAddedToCatalog, {
      category: 'Category',
      productKey: '1'
  ...
  test ('it updates an entry', async () => {
    await map.handle (plainToClass (ProductPriceSet, { productKey: '1', price: 12 }), {})
    expect (db['1']).toMatchObject ({ id: '1', category: 'Category', price: 12 })
  })

  test ('it throws when updating a non existent entry', async () => {
    await expect (map.handle (plainToClass (ProductPriceSet, {
      productKey: '2',
      price: 12
  ...
  test ('it deletes an entry', async () => {
    await map.handle (plainToClass (ProductRemovedFromCatalog, { productKey: '1' }), {})
    expect (db['1']).toBeUndefined ()
  })

  test ('it throws when deleting a non-existent entry', async () => {

    await expect (map.handle (plainToClass (ProductRemovedFromCatalog, { productKey: '2' }), {}))
...
describe ('Complete handling with ignores', () => {
  ...
  builder.map (ProductRemovedFromCatalog).asDeleteOf (x => x.productKey).ignoringMisses ()
  ...
  test ('it does nothing when creating the same entry', async () => {
    await map.handle (plainToClass (ProductAddedToCatalog, { category: 'OtherCategory', productKey: '1' }), {})
    expect (db['1']).toMatchObject ({ id: '1', category: 'Category' })
  })
  ...
  test ('it does nothing when updating a non-existent entry', async () => {
    await (map.handle (plainToClass (ProductPriceSet, {
      productKey: '2',
      price: 12
    }), {}))
  ...
  test ('it does nothing when deleting a non-existent entry', async () => {
    await map.handle (plainToClass (ProductRemovedFromCatalog, { productKey: '2' }), {})
    expect (db['2']).toBeUndefined ()
  })
})
describe ('Complete handling with overwrites', () => {
  ...
  test ('it updates an entry', async () => {
    await map.handle (plainToClass (ProductPriceSet, { productKey: '1', price: 12 }), {})
    expect (db['1']).toMatchObject ({ id: '1', category: 'OtherCategory', price: 12 })
  })
