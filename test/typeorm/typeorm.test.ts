...
import { buildEntityEventMap, generateEventsForASingleProduct, ProductCatalogEntry } from '../utils'
...
class MyProjector {
  ...
  private buildProjector() {
    this.projector = new TypeOrmProjector (
      ProductCatalogEntry,
      <T>(t: Type<T>) => this.tx?.getRepository (t) ?? getConnection ().getRepository (t),
      buildEntityEventMap (),
      (p, k) => {
        p.id = k
      })
  ...
  public async start() {
    ...
    event$.pipe (bufferTime (100), concatMap (async events => {
    ...
    await this.dispatcher.subscribe (lastCheckpoint, async event => event$.next (event))
  }
}
...
test ('It projects entities', async () => {
  ...
  const dispatcher = new Dispatcher (async (lastCheckpoint, handler, id) => subject.pipe (
  ...
  const events = generateEventsForASingleProduct ().map ((event, i): EventEnvelope => ({
    position: ++position,
    version: i,
  ...
  await expect (getConnection ().getRepository (ProductCatalogEntry).findOne ('1')).resolves.toMatchObject ({
    id: '1',
    category: 'MyCategory',
    price: 12
  })
